...
void Max7456::setBlinkParams(byte blinkBase, byte blinkDC) {
  ...
  osdSPI.transfer(VM1_ADDRESS_WRITE);
  osdSPI.transfer(_regVm1.whole);
...
void Max7456::setDisplayOffsets(byte horizontal, byte vertical) {
  ...
  digitalWrite(_pinCS, LOW);
  osdSPI.transfer(HOS_ADDRESS_WRITE);
  osdSPI.transfer(_regHos.whole);
  osdSPI.transfer(VOS_ADDRESS_WRITE);
  osdSPI.transfer(_regVos.whole);
  digitalWrite(_pinCS, HIGH);
}
...
void Max7456::sendCharacter(const charact chara, byte x, byte y) {
  ...
  // Datasheet page 38
  digitalWrite(_pinCS, LOW);
  osdSPI.transfer(CMAH_ADDRESS_WRITE);
  osdSPI.transfer(charAddress);

  for (byte i = 0; i < 54; i++) {
    osdSPI.transfer(CMAL_ADDRESS_WRITE);
    osdSPI.transfer(i);
    osdSPI.transfer(CMDI_ADDRESS_WRITE);
    osdSPI.transfer(chara[i]);
  }

  _regCmm = 0xA0; //To write the NVM array
  osdSPI.transfer(CMM_ADDRESS_WRITE);
  osdSPI.transfer(_regCmm);
  //while STAT[5] is not good, we wait.
  _regStat.bits.characterMemoryStatus = 1;
  while (_regStat.bits.characterMemoryStatus == 1) {
    osdSPI.transfer(STAT_ADDRESS_READ);
    _regStat.whole = osdSPI.transfer(0x00);
  }
...
void Max7456::getCharacter(charact chara, byte x, byte y) {
  ...
  for (byte i = 0; i < 54; i++) {
    ...
    osdSPI.transfer(CMDO_ADDRESS_READ); //read from device through osdSPI
    chara[i] = osdSPI.transfer(0x00);
  }
...
void Max7456::printMax7456Chars(byte chars[], byte size, byte x, byte y,
  ...
  digitalWrite(_pinCS, LOW);
  osdSPI.transfer(DMM_ADDRESS_WRITE);
  osdSPI.transfer(_regDmm.whole);
  // Datasheet page 31
  osdSPI.transfer(DMAH_ADDRESS_WRITE); // set start address high
  osdSPI.transfer(posAddressHI);

  osdSPI.transfer(DMAL_ADDRESS_WRITE); // set start address low
  osdSPI.transfer(posAddressLO);

  for (int i = 0; i < size; i++) {
    currentCharMax7456 = chars[i];
    osdSPI.transfer(DMDI_ADDRESS_WRITE);
    osdSPI.transfer(currentCharMax7456);
  }

  //end character (we're done).
  osdSPI.transfer(DMDI_ADDRESS_WRITE);
  osdSPI.transfer(0xff);
  digitalWrite(_pinCS, HIGH);
}
...
void Max7456::clearScreen() {
  ...
  while (_regDmm.bits.clearDisplayMemory == 1) {
    osdSPI.transfer(DMM_ADDRESS_READ);
    _regDmm.whole = osdSPI.transfer(0x00);
  }
...
//-----------------------------------------------------------------------------
void Max7456::init(byte iPinCS = OSD_PIN_CS) {
  osdSPI.begin(OSD_PIN_SCK, OSD_PIN_MISO, OSD_PIN_MOSI);
  osdSPI.setFrequency(500000);
  _pinCS = iPinCS;
  ...
  digitalWrite(_pinCS, LOW);
  osdSPI.transfer(VM0_ADDRESS_WRITE);

  _regVm0.whole = 0x00;
  _regVm0.bits.videoSelect = 1; //PAL
  _regVm0.bits.softwareResetBit = 1;
  osdSPI.transfer(_regVm0.whole);
  digitalWrite(_pinCS, HIGH);
  delay(500);
  ...
  for (int x = 0; x < 16; x++) {
    ...
    osdSPI.transfer(x + RB0_ADDRESS_WRITE);
    osdSPI.transfer(_regRb[x].whole);
  }
